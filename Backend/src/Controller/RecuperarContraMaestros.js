...
const recuperarContraMaestro = {}

recuperarContraMaestro.recuperarContra=async (req, res) => {
    try {

        const {email} = req.body
        const userFound = await maestroModal.findOne({email})
...
recuperarContraMaestro.verificarCodigo = async (req, res)=>{
    try {
        ...
        const newToken = JsonWebToken.sign(
            {email:decoded.email, userType:"Maestro", verified: true},
            config.JWT.SECRET,
            {expiresIn: "15m"},
...
recuperarContraMaestro.NewPassword = async (req, res) => {
    try {
    ...
    await maestroModal.findOneAndUpdate(
        {email:decoded.email},
...
export default recuperarContraMaestro